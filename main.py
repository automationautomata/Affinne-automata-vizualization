...
class Line:
    ...
    def calc(self, number):
        ...
        y = x*k + b
        return x, y
freecoef_2adic = float(Fraction('-1/3'))
frac = Fraction('1/3')
...
points = sorted(points, key=lambda vec: (vec[0]**2 + vec[1]**2)**0.5, reverse=frac.numerator<0)
...
for t in range(0, len(data)):
    ...
    # mesh = pv.Line(end, _toruscoords((array[0][0], array[1][0])))
